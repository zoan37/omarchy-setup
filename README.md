...
- [**XPS 16: fan spins up on video — measurement log, cause unproven**](xps16-fan-spins-up-on-video.md) — same symptom as the XPS 13, different machine, and the XPS 13's fix is a no-op here (VA-API is installed and working). Mostly a dead-ends list: EPP, a turbo clock cap, halved RAPL limits and the BIOS `Quiet` mode all fail to move the fan. Includes the sensor trap that hid the problem for hours (`dell_ddv` reads ~20 °C below `x86_pkg_temp`), the finding that fan state correlates *inversely* with package temperature, and the BIOS `ThermalManagement` attribute Linux can set without rebooting.
