...
- [**Zephyrus M16: quiet fans, low power, no desktop lag**](zephyrus-m16-quiet-power.md) — Quiet profile on AC and battery (Omarchy's own power-profile layer overrides asusd unless you set it too), a fan curve that stays at 0 RPM up to 66°C, and Integrated-only GPU because the RTX 3070 Ti never suspends in hybrid mode (~11 W idle). Also the catch that made the 165 Hz desktop stutter: EPP `power` capped the i9 at ~1.7 GHz, and power-profiles-daemon's `power-saver` reapplied it after every reboot. Fixed with a `--block-driver=intel_pstate` drop-in; boost back to 4.7 GHz, idle 37°C with fans off. Plus idle coil whine cut sharply by disabling the C8/C10 idle states (~1 W); a 30/35 W power cap and a 66°C fan start so scrolling X and YouTube stop spinning the fans (0 of 90 samples, down from 43); and a guard timer for the EC silently dropping the custom fan curve (after the BIOS 311 flash and on every asusd restart). Exact commands, verification, and revert steps.
