...
| Dell XPS 16 (DA16260) | Intel Panther Lake (Arc B390) | SKU `0DBA`. **Being returned (2026-09-19): the zero-lattice keyboard never felt right (see [keyboard notes](xps16-keyboard-dropped-keys.md)), the short fan bursts at idle were part of it too, and so was poor palm rejection (the touchpad reports neither contact size nor a pressure that separates palms from fingers, see [section 3 of the keyboard notes](xps16-keyboard-dropped-keys.md#3-touchpad-palm-detection-by-pressure-not-possible), so a custom palm filter wasn't feasible); the bursts were later traced to a browser page holding a core, not the hardware (see [fan notes](xps16-fan-spins-up-on-video.md)).** 3200x2000@120Hz LG **OLED** eDP, fractional scale 1.6 (stock `auto` resolved correctly). Fresh Quattro 4.0.4 install. Assess display issues separately from the XPS 13; live Panel Replay checks in [xps16-notes.md](xps16-notes.md). |
...
- [**Zenbook A16: Omarchy Snapdragon install and fixes**](zenbook-a16-omarchy-snapdragon.md) — installer clock bug (set the date before `pacman -Syy`), QCC2072 Wi-Fi board file rebuilt from the ASUS driver package, UCM file-name symlink that brings up speakers and mic, a device-tree patch inside `vmlinuz.efi` that enables CPU frequency scaling, custom GRUB entries, a userspace quiet thermal profile, the embedded-controller reverse engineering (Linux I2C + Windows WMI) that ended in manual fan control, a Mac-style "whisper" fan daemon (0 rpm idle), the Windows 11 dual boot via ASUS Cloud Recovery, BIOS updates on a Snapdragon model, a coil whine traced with a webcam mic and a pure-Python FFT to the SSD's PCIe link power state (fixed with one sysfs bit), silent tweeters (stock PipeWire leaves the 4-channel sink's rear pair, the tweeters, unfed; mic-mapped, sides crossed) fixed with a filter-chain crossover plus a limited +6 dB boost, a GPU clock cap that stops the Adreno driver boosting to 1.85 GHz on trivial desktop redraws (2–3 W of a ~13–15 W light-use drain; the rest is missing Glymur power management), a behavioral touchpad palm filter (keystroke timing, landing zone and travel; the pad has no size/pressure) that re-emits accepted fingers on a virtual pad without muting it for held game keys, and the things that did not work. Scripts, Windows PowerShell helpers and EC dumps in `assets/zenbook-a16/`.
