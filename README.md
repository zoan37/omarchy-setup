...
- [**Zephyrus M16: quiet fans, low power, no desktop lag**](zephyrus-m16-quiet-power.md) — Quiet profile on AC and battery (Omarchy's own power-profile layer overrides asusd unless you set it too), a steady ~2,000 RPM baseline fan curve (the EC's hardware minimum), and Integrated-only GPU because the RTX 3070 Ti never suspends in hybrid mode (~11 W idle). Also the catch that made the 165 Hz desktop stutter: EPP `power` capped the i9 at ~1.7 GHz, and power-profiles-daemon's `power-saver` reapplied it after every reboot. Fixed with a `--block-driver=intel_pstate` drop-in; boost back to 4.7 GHz, idle 37°C with fans off. Plus idle coil whine cut sharply by disabling the C8/C10 idle states (~1 W); a 30/35 W power cap, then a steady baseline fan instead of start/stop bursts (the fan can't spin below ~2,000 RPM); a 3.8 GHz clock cap that took peak temps from 73°C to 61°C, plus VRR on and panel overdrive off; and a guard timer for the EC silently dropping the custom fan curve (after the BIOS 311 flash and on every asusd restart). Exact commands, verification, and revert steps.
...
- [**SER8 Chrome toolbar flicker: OpenGL ANGLE + Vulkan trial**](chrome-ser8-flicker-trial.md) — retain WebGPU game support while changing ANGLE to OpenGL. MechaBlade's tutorial passed a short check; daily-browser flicker and video compatibility remain unverified. Exact flags, full-restart instructions, and rollback.
