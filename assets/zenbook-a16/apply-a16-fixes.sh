...
install -m 644 "$D/a16-cpuidle-nosleep.service" /etc/systemd/system/a16-cpuidle-nosleep.service   # mic: no effect; ear: helps; enabled
...
install -m 755 "$D/a16-whine-tweaks" /usr/local/bin/a16-whine-tweaks
install -m 644 "$D/a16-whine-tweaks.service" /etc/systemd/system/a16-whine-tweaks.service
[ -e /etc/default/a16-whine ] || install -m 644 "$D/a16-whine.conf" /etc/default/a16-whine
systemctl daemon-reload; systemctl enable a16-quiet-thermal.service a16-fan-daemon.service a16-nvme-aspm.service a16-whine-tweaks.service a16-cpuidle-nosleep.service >/dev/null 2>&1 || true
